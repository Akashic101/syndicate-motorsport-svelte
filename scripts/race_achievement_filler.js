...
async function backfillAchievements() {
	...
	const { data: drivers, error: driverError } = await supabase
		.from('driver_data')
		.select('driver_guid, race_starts')
	...
	for (const driver of drivers) {
		const { driver_guid, race_starts } = driver;

		if (!race_starts || race_starts <= 0) continue;

		// achievements this driver qualifies for
		const eligible = achievements.filter((a) => race_starts >= a.threshold);

...
async function updateSheet(sheet) {
	try {
		...
		console.log(
			`🏗️  Ensuring table '${sheet.name}' exists with columns:`,
			sheet.columns.map((c) => `${c.name} (${c.type})`)
		);
		...
		db.serialize(() => {
			...
			stmt.finalize((err) => {
				if (err) {
					...
					console.log(
						`✅ Completed sync for '${sheet.name}': ${successCount} successful, ${errorCount} errors`
					);
				}
			});
		});
